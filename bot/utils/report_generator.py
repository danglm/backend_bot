...
Render báo cáo tổng hợp xưởng thành ảnh PNG bằng HTML/CSS + Playwright.
"""
import io
import datetime
...
def build_report_html(
    ...
    """Tạo HTML báo cáo thu mua của Xưởng."""
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .brand-name {{
    ...
    color: #e2e8f0; letter-spacing: 1.5px;
  ...
  <div class="report-box">
    <div class="att-header">
      <div class="header-top">
        <div>
          <div class="brand-name">BÁO CÁO NHÀ CUNG CẤP</div>
          <div class="brand-sub">{timeframe}</div>
        </div>
        <div class="header-badge">{target_name}</div>
      </div>
      <div class="att-title">
        <h1>CHI TIẾT MUA MỦ THEO HỘ</h1>
      </div>
    </div>
    <div class="tear"></div>
    <div class="att-body">
      <div class="summary-info">
        ...
        <div class="info-block" style="text-align: right;">
          ...
          <span class="info-val val-green">{total_amount}</span>
        </div>
      </div>
      <table>
        <thead>
...
def _render_report_to_png_sync(html_content: str, max_retries: int = 2) -> bytes:
...
async def render_report_to_png(html_content: str) -> bytes:
    import asyncio
    return await asyncio.to_thread(_render_report_to_png_sync, html_content)
...
async def generate_report_image(
    ...
    html = build_report_html(target_name, timeframe, total_weight, total_amount, records)
    png_bytes = await render_report_to_png(html)

    buf = io.BytesIO(png_bytes)
    buf.name = f"baocao_{target_name}.png"
    return buf
