"""
Save Report Image Generator
Render báo cáo thống kê lưu sổ thành ảnh PNG bằng HTML/CSS + Playwright.
"""
import io
import datetime


def build_save_report_html(
    target_name: str,
    timeframe: str,
    total_weight: str,
    total_saved: str,
    ...
    """Tạo HTML báo cáo lưu sổ của Xưởng."""
...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .brand-name {{
    ...
    color: #fde68a; letter-spacing: 1.5px;
  ...
  tr.even {{ background: #fffbeb; }}
  tr.odd {{ background: #ffffff; }}
  .success {{ color: #b45309; font-weight: 700; }}
    ...
    <div class="att-body">
      <div class="summary-info">
        ...
        <div class="info-block" style="text-align: right;">
          <span class="info-label">Tổng lưu sổ</span>
          <span class="info-val val-amber">{total_saved}</span>
        </div>
      </div>
      <table>
        <thead>
          <tr>
            ...
            <th style="text-align: right;">Lưu Sổ</th>
          </tr>
        </thead>
        <tbody>
          {table_rows}
...
async def render_save_report_to_png(html_content: str) -> bytes:
    import asyncio
    return await asyncio.to_thread(_render_save_report_to_png_sync, html_content)


async def generate_save_report_image(
    target_name: str,
    timeframe: str,
    total_weight: str,
    total_saved: str,
    ...
    html = build_save_report_html(target_name, timeframe, total_weight, total_saved, records)
    png_bytes = await render_save_report_to_png(html)

    buf = io.BytesIO(png_bytes)
    buf.name = f"luuso_{target_name}.png"
    return buf
