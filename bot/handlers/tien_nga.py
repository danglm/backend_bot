...
async def tien_nga_create_customer_handler(client, message: Message) -> None:
    ...
    try:
        ...
        new_customer = Customers(
            ...
            status=status.upper(),
            is_subsidized=int(is_subsidized)
        )
        db.add(new_customer)
        db.commit()
...
async def tien_nga_export_daily_purchase_handler(client, message: Message) -> None:
    ...
    try:
        ...
        for p in purchases:
            ...
            ung_tien = p.advance_amount or 0
            ...
            records.append({
                ...
                "gia_ht": p.subsidy_price or ((p.unit_price or 0) + (p.is_subsidized or 0)),
    ...
    try:
        ...
        for p in purchases:
            ...
            records.append({
                ...
                "mu_kho": mu_kho,
                "don_gia": don_gia,
                "gia_ht": p.subsidy_price or ((don_gia or 0) + (p.is_subsidized or 0)),
                "thanh_tien": thanh_tien,
...
@bot.on_message(filters.command(["tien_nga_export_paid_bill", "tien_nga_xuat_hoa_don_da_tt"]) | filters.regex(r"^@\w+\s+/(tien_nga_export_paid_bill|tien_nga_xuat_hoa_don_da_tt)\b"))
@require_user_type(UserType.OWNER, UserType.ADMIN, UserType.MEMBER)
@require_project_name("Tiến Nga")
@require_group_role("main", "member")
@require_custom_title(CustomTitle.SUPER_MAIN, CustomTitle.MAIN_SUPPLIER, CustomTitle.MEMBER_SUPPLIER)
async def tien_nga_export_paid_bill_handler(client, message: Message) -> None:
    ...
    loading_msg = await message.reply_text("⏳ Đang tạo hóa đơn đã thanh toán, vui lòng chờ...", parse_mode=ParseMode.HTML)
    ...
    try:
        ...
        # Query purchases with paid_amount > 0
        purchases = (
            db.query(DailyPurchases)
            .filter(
                DailyPurchases.hoursehold_id == hoursehold_id,
        ...
        if not purchases:
            await loading_msg.delete()
            await message.reply_text(
                f"ℹ️ Không có dữ liệu đã thanh toán cho <b>{customer.fullname}</b> ({hoursehold_id}) trong khoảng <b>{timeframe}</b>.",
                parse_mode=ParseMode.HTML
            )
            return
        ...
        try:
            ...
            await message.reply_document(
                document=img_buf,
                file_name=f"hoa_don_da_thanh_toan_{hoursehold_id}.png",
                caption=caption_text,
                parse_mode=ParseMode.HTML,
            )

        await loading_msg.delete()
        LogInfo(f"[TienNga] Exported paid bill report for '{hoursehold_id}' ({timeframe}) by user {message.from_user.id}", LogType.SYSTEM_STATUS)

    except Exception as e:
        LogError(f"Error exporting paid bill: {e}", LogType.SYSTEM_STATUS)
        import traceback
        traceback.print_exc()
        await loading_msg.delete()
        await message.reply_text("❌ Có lỗi xảy ra khi tạo hóa đơn đã thanh toán.", parse_mode=ParseMode.HTML)
    finally:
        db.close()


# ═══════════════════════════════════════════════════════════════
# CALLBACK HANDLERS cho nút chọn thời gian báo cáo thu mua / lưu sổ / đã TT
# ═══════════════════════════════════════════════════════════════

def _resolve_report_dates(period: str):
    """Chuyển đổi tên chu kỳ thành (start_date, end_date, timeframe_label)."""
    ...
    if period == "today":
        ...
        return yesterday, yesterday, yesterday.strftime("%d/%m/%Y")
    elif period == "week":
...
# --- Báo cáo thu mua (rpt_tm) ---

@bot.on_callback_query(filters.regex(r"^rpt_tm_cancel$"))
async def rpt_tm_cancel_callback(client, callback_query):
    await callback_query.message.delete()
    await callback_query.answer("Đã hủy.")


@bot.on_callback_query(filters.regex(r"^rpt_tm\|([^|]+)\|custom$"))
async def rpt_tm_custom_callback(client, callback_query):
    hoursehold_id = callback_query.matches[0].group(1)
    await callback_query.message.edit_text(
        f"📝 Vui lòng nhập lệnh với khoảng thời gian tùy chọn:\n\n"
        f"<code>/tien_nga_xuat_bao_cao_thu_mua {hoursehold_id} dd/mm/yyyy - dd/mm/yyyy</code>\n\n"
        f"<i>Ví dụ:\n"
        f"<code>/tien_nga_xuat_bao_cao_thu_mua {hoursehold_id} 01/07/2026 - 31/07/2026</code></i>",
        parse_mode=ParseMode.HTML
    )
    await callback_query.answer()


@bot.on_callback_query(filters.regex(r"^rpt_tm\|([^|]+)\|(today|yesterday|week|month|year)$"))
async def rpt_tm_period_callback(client, callback_query):
    hoursehold_id = callback_query.matches[0].group(1)
    period = callback_query.matches[0].group(2)

    start_date, end_date, timeframe = _resolve_report_dates(period)
    if not start_date:
        await callback_query.answer("⚠️ Chu kỳ không hợp lệ.", show_alert=True)
        return

    await callback_query.message.edit_text(
        f"⏳ Đang tạo báo cáo thu mua cho <b>{hoursehold_id}</b>...\n"
        f"Thời gian: <b>{timeframe}</b>",
        parse_mode=ParseMode.HTML
    )
    await callback_query.answer()
    ...
    try:
        ...
        await callback_query.message.delete()
        LogInfo(f"[TienNga] Exported daily purchase report for '{hoursehold_id}' ({timeframe}) via button by user {callback_query.from_user.id}", LogType.SYSTEM_STATUS)

    except Exception as e:
        LogError(f"Error exporting daily purchase (callback): {e}", LogType.SYSTEM_STATUS)
        traceback.print_exc()
...
# --- Hóa đơn lưu sổ (rpt_ls) ---

@bot.on_callback_query(filters.regex(r"^rpt_ls_cancel$"))
async def rpt_ls_cancel_callback(client, callback_query):
    await callback_query.message.delete()
    await callback_query.answer("Đã hủy.")


@bot.on_callback_query(filters.regex(r"^rpt_ls\|([^|]+)\|custom$"))
async def rpt_ls_custom_callback(client, callback_query):
    hoursehold_id = callback_query.matches[0].group(1)
    await callback_query.message.edit_text(
        f"📝 Vui lòng nhập lệnh với khoảng thời gian tùy chọn:\n\n"
        f"<code>/tien_nga_xuat_hoa_don_luu_so {hoursehold_id} dd/mm/yyyy - dd/mm/yyyy</code>\n\n"
        f"<i>Ví dụ:\n"
        f"<code>/tien_nga_xuat_hoa_don_luu_so {hoursehold_id} 01/07/2026 - 31/07/2026</code></i>",
        parse_mode=ParseMode.HTML
    )
    await callback_query.answer()


@bot.on_callback_query(filters.regex(r"^rpt_ls\|([^|]+)\|(today|yesterday|week|month|year)$"))
async def rpt_ls_period_callback(client, callback_query):
    hoursehold_id = callback_query.matches[0].group(1)
    period = callback_query.matches[0].group(2)
    ...
    await callback_query.message.edit_text(
        f"⏳ Đang tạo hóa đơn lưu sổ cho <b>{hoursehold_id}</b>...\n"
        f"Thời gian: <b>{timeframe}</b>",
        parse_mode=ParseMode.HTML
    )
    await callback_query.answer()
    ...
    try:
        ...
        await callback_query.message.delete()
        LogInfo(f"[TienNga] Exported saved bill report for '{hoursehold_id}' ({timeframe}) via button by user {callback_query.from_user.id}", LogType.SYSTEM_STATUS)

    except Exception as e:
        LogError(f"Error exporting saved bill (callback): {e}", LogType.SYSTEM_STATUS)
        traceback.print_exc()
        await callback_query.message.edit_text("❌ Có lỗi xảy ra khi tạo hóa đơn lưu sổ.", parse_mode=ParseMode.HTML)
    finally:
        db.close()


# --- Hóa đơn đã thanh toán (rpt_tt) ---

@bot.on_callback_query(filters.regex(r"^rpt_tt_cancel$"))
async def rpt_tt_cancel_callback(client, callback_query):
    await callback_query.message.delete()
    await callback_query.answer("Đã hủy.")


@bot.on_callback_query(filters.regex(r"^rpt_tt\|([^|]+)\|custom$"))
async def rpt_tt_custom_callback(client, callback_query):
    hoursehold_id = callback_query.matches[0].group(1)
    await callback_query.message.edit_text(
        f"📝 Vui lòng nhập lệnh với khoảng thời gian tùy chọn:\n\n"
        f"<code>/tien_nga_xuat_hoa_don_da_tt {hoursehold_id} dd/mm/yyyy - dd/mm/yyyy</code>\n\n"
        f"<i>Ví dụ:\n"
        f"<code>/tien_nga_xuat_hoa_don_da_tt {hoursehold_id} 01/07/2026 - 31/07/2026</code></i>",
        parse_mode=ParseMode.HTML
    )
    await callback_query.answer()


@bot.on_callback_query(filters.regex(r"^rpt_tt\|([^|]+)\|(today|yesterday|week|month|year)$"))
async def rpt_tt_period_callback(client, callback_query):
    hoursehold_id = callback_query.matches[0].group(1)
    period = callback_query.matches[0].group(2)
    ...
    await callback_query.message.edit_text(
        f"⏳ Đang tạo hóa đơn đã thanh toán cho <b>{hoursehold_id}</b>...\n"
        f"Thời gian: <b>{timeframe}</b>",
        parse_mode=ParseMode.HTML
    )
    await callback_query.answer()
    ...
    try:
        ...
        await callback_query.message.delete()
        LogInfo(f"[TienNga] Exported paid bill report for '{hoursehold_id}' ({timeframe}) via button by user {callback_query.from_user.id}", LogType.SYSTEM_STATUS)

    except Exception as e:
        LogError(f"Error exporting paid bill (callback): {e}", LogType.SYSTEM_STATUS)
        traceback.print_exc()
        await callback_query.message.edit_text("❌ Có lỗi xảy ra khi tạo hóa đơn đã thanh toán.", parse_mode=ParseMode.HTML)
    finally:
        db.close()
...
async def send_message_handler(client, message: Message) -> None:
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("Tất cả", callback_data="sm_proj_all")],
        [InlineKeyboardButton("Tiến Nga", callback_data="sm_proj_tiennga")],
        [InlineKeyboardButton("Credit", callback_data="sm_proj_credit"),
         InlineKeyboardButton("Rental", callback_data="sm_proj_rental")],
        [InlineKeyboardButton("Ggomoosin", callback_data="sm_proj_ggomoosin"),
         InlineKeyboardButton("Other", callback_data="sm_proj_other")],
        [InlineKeyboardButton("Hụi", callback_data="sm_proj_hui"),
         InlineKeyboardButton("Thu Hoạch", callback_data="sm_proj_thuhoach")],
...
async def send_message_callback(client, callback_query: CallbackQuery):
    ...
    # === Gửi tất cả nhóm member tất cả dự án ===
    elif data == "sm_proj_all":
        await callback_query.message.edit_text("⏳ <i>Đang gửi tin nhắn đến tất cả nhóm member...</i>", parse_mode=ParseMode.HTML)
        
        sent, failed = await _do_send_to_all_projects(client, state)
        
        _send_msg_state.pop(user_id, None)
        
        await callback_query.message.edit_text(
            f"✅ <b>ĐÃ GỬI TIN NHẮN</b>\n\n"
            f"Nhóm: <b>Tất cả dự án</b>\n"
            f"Gửi thành công: <b>{sent}</b> nhóm\n"
            f"Thất bại: <b>{failed}</b> nhóm",
            parse_mode=ParseMode.HTML
        )
    
    # === Dự án cụ thể -> Sub-menu (Gửi tất cả / Nhóm Main / Nhóm Member) ===
    elif data in ("sm_proj_credit", "sm_proj_rental", "sm_proj_ggomoosin", "sm_proj_other", "sm_proj_hui", "sm_proj_thuhoach"):
        proj_map = {
            "sm_proj_credit": ("credit", "Credit"),
            "sm_proj_rental": ("rental", "Rental"),
            "sm_proj_ggomoosin": ("ggo", "Ggomoosin"),
            "sm_proj_other": ("other", "Other"),
            "sm_proj_hui": ("hui", "Hụi"),
            "sm_proj_thuhoach": ("th", "Thu Hoạch"),
        }
        proj_key, project_name = proj_map[data]
        state["current_proj_key"] = proj_key
        state["current_proj_name"] = project_name
        state["selected"] = set()
        
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("Gửi tất cả nhóm", callback_data=f"sm_gp_all|{proj_key}")],
            [InlineKeyboardButton("Nhóm Main", callback_data=f"sm_gp_sel|{proj_key}|main"),
             InlineKeyboardButton("Nhóm Member", callback_data=f"sm_gp_sel|{proj_key}|member")],
            [InlineKeyboardButton("Hủy", callback_data="sm_cancel"),
             InlineKeyboardButton("Quay lại", callback_data="sm_back_proj")],
        ])
        await callback_query.message.edit_text(
            f"<b>DỰ ÁN: {project_name.upper()}</b>\n\nChọn nhóm nhận tin:",
            parse_mode=ParseMode.HTML, reply_markup=kb
        )
    
    # === Gửi tất cả nhóm trong 1 dự án ===
    elif data.startswith("sm_gp_all|"):
        proj_key = data.split("|")[1]
        project_name = state.get("current_proj_name", proj_key)
        
        await callback_query.message.edit_text(
            f"⏳ <i>Đang gửi tin nhắn đến tất cả nhóm dự án {project_name}...</i>",
            parse_mode=ParseMode.HTML
        )
        
        sent, failed = await _do_send_to_groups(client, state, project_name, None)
    ...
    # === Hiển thị multi-select nhóm Main / Member ===
    elif data.startswith("sm_gp_sel|"):
        parts = data.split("|")
        proj_key = parts[1]
        role_filter = parts[2]  # "main" or "member"
        
        state["current_proj_key"] = proj_key
        state["gp_role"] = role_filter
        state["selected"] = set()
        
        await _render_group_select(callback_query, state)
    
    # === Toggle chọn nhóm ===
    elif data.startswith("sm_gp_tog|"):
        idx = data.split("|")[1]
        selected = state.get("selected", set())
        if idx in selected:
    ...
    # === Gửi đến các nhóm đã chọn ===
    elif data == "sm_gp_send":
        selected = state.get("selected", set())
        if not selected:
            await callback_query.answer("⚠️ Vui lòng chọn ít nhất 1 nhóm!", show_alert=True)
            return
        
        groups = state.get("gp_groups", [])
        chat_ids = [g[0] for g in groups if g[0] in selected]
        project_name = state.get("current_proj_name", "")
        role_label = "Main" if state.get("gp_role") == "main" else "Member"
        
        await callback_query.message.edit_text(
            f"⏳ <i>Đang gửi tin nhắn đến {len(chat_ids)} nhóm {role_label}...</i>",
            parse_mode=ParseMode.HTML
        )
        
        sent, failed = await _do_send_to_chat_ids(client, state, chat_ids)
        
        _send_msg_state.pop(user_id, None)
        
        await callback_query.message.edit_text(
            f"✅ <b>ĐÃ GỬI TIN NHẮN</b>\n\n"
            f"Dự án: <b>{project_name}</b> ({role_label})\n"
            f"Gửi thành công: <b>{sent}</b> nhóm\n"
            f"Thất bại: <b>{failed}</b> nhóm",
            parse_mode=ParseMode.HTML
        )
    
    # === Quay lại sub-menu dự án ===
    elif data.startswith("sm_gp_back|"):
        proj_key = data.split("|")[1]
        project_name = state.get("current_proj_name", proj_key)
        state["selected"] = set()
...
async def _render_group_select(callback_query, state):
    """Render multi-select buttons for Main/Member groups in a project."""
    from app.db.session import SessionLocal
    from app.models.business import Projects
    from app.models.telegram import TelegramProjectMember
    
    proj_key = state.get("current_proj_key", "")
    project_name = state.get("current_proj_name", "")
    role_filter = state.get("gp_role", "member")
    
    proj_key_map = {
        "credit": "Credit", "rental": "Rental", "ggo": "Ggomoosin",
        "other": "Other", "hui": "Hụi", "th": "Thu Hoạch",
    }
    db_project_name = proj_key_map.get(proj_key, project_name)
    
    db = SessionLocal()
    try:
        project = db.query(Projects).filter(Projects.project_name == db_project_name).first()
        if not project:
            await callback_query.answer(f"⚠️ Không tìm thấy dự án {db_project_name}!", show_alert=True)
            return
        
        # Query distinct groups by role
        groups_raw = db.query(
            TelegramProjectMember.chat_id,
            TelegramProjectMember.group_name
        ).filter(
            TelegramProjectMember.project_id == project.id,
            TelegramProjectMember.role == role_filter
        ).distinct().all()
        
        groups = [(str(g.chat_id), g.group_name or f"Nhóm {g.chat_id}") for g in groups_raw]
    except Exception as e:
        LogError(f"[SendMsg] Error querying groups: {e}", LogType.SYSTEM_STATUS)
        groups = []
    finally:
        db.close()
    
    if not groups:
        role_label = "Main" if role_filter == "main" else "Member"
        await callback_query.answer(f"⚠️ Không tìm thấy nhóm {role_label} nào trong dự án {project_name}!", show_alert=True)
        return
    
    # Store groups in state for toggle/send
    state["gp_groups"] = groups
    
    selected = state.get("selected", set())
    
    buttons = []
    for chat_id, label in groups:
        check = "✅" if chat_id in selected else "⬜"
        buttons.append([InlineKeyboardButton(f"{check} {label}", callback_data=f"sm_gp_tog|{chat_id}")])
    
    buttons.append([InlineKeyboardButton("Gửi", callback_data="sm_gp_send")])
    buttons.append([
        InlineKeyboardButton("Hủy", callback_data="sm_cancel"),
        InlineKeyboardButton("Quay lại", callback_data=f"sm_gp_back|{proj_key}"),
    ])
    
    selected_names = [g[1] for g in groups if g[0] in selected]
    selected_text = ", ".join(selected_names) if selected_names else "Chưa chọn"
    role_label = "NHÓM MAIN" if role_filter == "main" else "NHÓM MEMBER"
    
    kb = InlineKeyboardMarkup(buttons)
    await callback_query.message.edit_text(
        f"<b>{project_name.upper()} — {role_label}</b>\n\n"
        f"Đã chọn: <b>{selected_text}</b>\n\n"
        f"Nhấn để chọn/bỏ chọn, sau đó nhấn <b>Gửi</b>:",
        parse_mode=ParseMode.HTML, reply_markup=kb
    )
...
async def _do_send_to_chat_ids(client, state, chat_ids: list):
    """
    Gửi tin nhắn/ảnh/video đến danh sách chat_ids cụ thể.
...
async def _do_send_to_all_projects(client, state):
    """
    Gửi tin nhắn/ảnh/video đến tất cả nhóm member của tất cả dự án.
    Returns (sent_count, failed_count).
    """
    from app.db.session import SessionLocal
    from app.models.telegram import TelegramProjectMember
    ...
    try:
        # Lấy tất cả nhóm member (không phân biệt dự án)
        ...
        LogError(f"[SendMsg] Error sending to all projects: {e}", LogType.SYSTEM_STATUS)
    finally:
        db.close()
...
# Helper functions for Partner Keyboard
def _build_tn_dt_main_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Xem danh sách đối tác", callback_data="tn_dt_list|0")],
        [InlineKeyboardButton("Tạo đối tác mới", callback_data="tn_dt_form")],
        [InlineKeyboardButton("Hủy", callback_data="tn_dt_cancel")]
    ])

def _build_tn_dt_list_keyboard(total_items: int, page: int):
    PAGE_SIZE = 10
    ...
    nav_row = []
    if page > 0:
        nav_row.append(InlineKeyboardButton("<< Trước", callback_data=f"tn_dt_list|{page - 1}"))
    nav_row.append(InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="tn_dt_noop"))
    if page < total_pages - 1:
        nav_row.append(InlineKeyboardButton("Sau >>", callback_data=f"tn_dt_list|{page + 1}"))
        
    buttons.append(nav_row)
    buttons.append([
        InlineKeyboardButton("Trở lại", callback_data="tn_dt_menu"),
        InlineKeyboardButton("Hủy", callback_data="tn_dt_cancel")
    ])
    return InlineKeyboardMarkup(buttons)

@bot.on_callback_query(filters.regex(r"^tn_dt_noop$"))
async def tn_dt_noop_callback(client, callback_query: CallbackQuery):
    await callback_query.answer()

@bot.on_callback_query(filters.regex(r"^tn_dt_cancel$"))
async def tn_dt_cancel_callback(client, callback_query: CallbackQuery):
    await callback_query.message.delete()

@bot.on_callback_query(filters.regex(r"^tn_dt_menu$"))
async def tn_dt_menu_callback(client, callback_query: CallbackQuery):
    menu_text = "<b>QUẢN LÝ ĐỐI TÁC TIẾN NGA</b>\n\nVui lòng chọn thao tác:"
    await callback_query.message.edit_text(menu_text, reply_markup=_build_tn_dt_main_keyboard(), parse_mode=ParseMode.HTML)

@bot.on_callback_query(filters.regex(r"^tn_dt_form$"))
async def tn_dt_form_callback(client, callback_query: CallbackQuery):
    form_template = (
    ...
    cancel_kb = InlineKeyboardMarkup([[InlineKeyboardButton("Hủy", callback_data="tn_dt_cancel")]])
    await callback_query.message.edit_text(form_template, parse_mode=ParseMode.HTML, reply_markup=cancel_kb)

@bot.on_callback_query(filters.regex(r"^tn_dt_list\|(\d+)$"))
async def tn_dt_list_callback(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        if not partners:
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("Trở lại", callback_data="tn_dt_menu"), InlineKeyboardButton("Hủy", callback_data="tn_dt_cancel")]
            ])
            await callback_query.message.edit_text("<b>DANH SÁCH ĐỐI TÁC</b>\n\n<i>ℹ️ Chưa có đối tác nào trong hệ thống.</i>", parse_mode=ParseMode.HTML, reply_markup=kb)
            return
        ...
        page_partners = partners[start:start + PAGE_SIZE]

        lines = [f"<b>DANH SÁCH ĐỐI TÁC</b> (Tổng: <b>{total}</b> đối tác)\n"]
        for idx, p in enumerate(page_partners, start=start + 1):
            lines.append(f"{idx}. <b>{p.partner_id}</b> - {p.partner_name or 'N/A'}")

        text = "\n".join(lines)
        await callback_query.message.edit_text(text, reply_markup=_build_tn_dt_list_keyboard(total, page), parse_mode=ParseMode.HTML)
    except Exception as e:
        LogError(f"Error in tn_dt_list_callback: {e}", LogType.SYSTEM_STATUS)
...
async def tien_nga_create_partner_handler(client, message: Message) -> None:
    ...
    if len(lines) < 2:
        menu_text = "<b>QUẢN LÝ ĐỐI TÁC TIẾN NGA</b>\n\nVui lòng chọn thao tác:"
        await message.reply_text(menu_text, reply_markup=_build_tn_dt_main_keyboard(), parse_mode=ParseMode.HTML)
        return
...
async def tien_nga_payment_of_debt_handler(client, message: Message) -> None:
    ...
    from app.models.business import Customers, Partners, Households
    from app.models.employee import Employee
    ...
    try:
        ...
        if old_debt > 0:
            info_msg += f"<i>💡 Công nợ đang dương → Mình đang nợ {type_label.lower()}.</i>\n"
        elif old_debt < 0:
            info_msg += f"<i>💡 Công nợ đang âm → {type_label} đang nợ mình.</i>\n"
        else:
            info_msg += f"<i>💡 Công nợ hiện tại bằng 0.</i>\n"

        info_msg += "\n<b>Vui lòng chọn hành động:</b>"

        # callback data format: cb_ttcn_{action}_{target_type}_{target_id}_{amount}
        buttons = [
            [
                InlineKeyboardButton("Thu Công Nợ", callback_data=f"cb_ttcn_thu_{target_type}_{target_id}_{amount}"),
                InlineKeyboardButton("Trả Công Nợ", callback_data=f"cb_ttcn_tra_{target_type}_{target_id}_{amount}"),
            ],
            [InlineKeyboardButton("Hủy", callback_data="cb_ttcn_cancel")],
        ]
        ...
        await message.reply_text("❌ Hệ thống gặp lỗi.", parse_mode=ParseMode.HTML)
    finally:
        db.close()


# --- Cancel thanh toán công nợ ---
@bot.on_callback_query(filters.regex(r"^cb_ttcn_cancel$"))
async def ttcn_cancel_callback(client, callback_query: CallbackQuery):
    await callback_query.message.delete()
...
async def ttcn_action_callback(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        if target_type == "partner":
            entity = db.query(Partners).filter(Partners.partner_id == target_id).first()
            if entity:
                old_debt = entity.total_debt or 0
                name = entity.partner_name or entity.partner_id
        elif target_type == "household":
            entity = db.query(Households).filter(Households.household_code == target_id).first()
            if entity:
                old_debt = entity.total_debt or 0
                name = entity.fullname or entity.household_code
        elif target_type == "customer":
            entity = db.query(Customers).filter(Customers.hoursehold_id == target_id).first()
            if entity:
                old_debt = entity.total_debt or 0
                name = entity.fullname or entity.hoursehold_id
        elif target_type == "employee":
            entity = db.query(Employee).filter(Employee.id == target_id).first()
            if entity:
                old_debt = entity.total_debt or 0
        ...
        if action == "thu":
            # Thu Công Nợ: Đối tác đang nợ mình (debt âm), họ trả tiền → debt tăng (cộng amount)
            new_debt = old_debt + amount
            action_label = "THU CÔNG NỢ"
            action_desc = "Thu từ"
        else:
            # Trả Công Nợ: Mình đang nợ đối tác (debt dương), mình trả → debt giảm (trừ amount)
            new_debt = old_debt - amount
            action_label = "TRẢ CÔNG NỢ"
            action_desc = "Trả cho"
        ...
        thu_chi_type = "thu" if action == "thu" else "chi"
        suggest_msg = (
            f"<b>GỢI Ý:</b> Sao chép lệnh bên dưới để ghi nhận phiếu {thu_chi_type}:\n\n"
            f"<pre>/tien_nga_yeu_cau_thu_chi</pre>\n\n"
            f"<i>Sau khi chọn quỹ đầu tư, điền thông tin:\n"
            f"• Số Tiền (VNĐ): {fmt_amount}\n"
            f"• Người Nhận: {name}\n"
            f"• Mục Đích: {action_label} {type_label} {target_id}\n"
            f"• Lý Do: {action_label}</i>"
        )
        await callback_query.message.reply_text(suggest_msg, parse_mode=ParseMode.HTML)

        LogInfo(f"[TienNga] /tien_nga_payment_of_debt {action} {target_type} {target_id} amount={amount} by {callback_query.from_user.id}", LogType.SYSTEM_STATUS)

        # Notify member group
        notify_text = (
            f"🔔 <b>THÔNG BÁO CẬP NHẬT CÔNG NỢ</b>\n\n"
            f"<b>Kính gửi:</b> {name}\n"
            f"<b>Hành động:</b> {action_label}\n"
            f"<b>Số Tiền:</b> <code>{fmt_vn(amount)}</code>\n"
            f"<b>Công Nợ Hiện Tại:</b> <code>{fmt_vn(new_debt)}</code>\n\n"
            f"<i>Cảm ơn quý {type_label.lower()}!</i>"
        )
        ...
        if project:
            if target_type == "partner" and entity.telegram_group:
                member_chat = db.query(TelegramProjectMember).filter(
                    TelegramProjectMember.project_id == project.id,
                    TelegramProjectMember.role == "member",
...
Loại cây trồng mặc định: cao_su hoặc sau_rieng.</i>"""
    ...
    harvest_area = parse_float_vn(data.get("DT Đang Thu Hoạch (ha)", data.get("DT Thu Hoạch (ha)", data.get("DT Đang Thu Hoạch Cao Su (ha)", "0"))))
...
SL Cây Đang Trồng: {land.planting_trees or 0}</pre>"""
    ...
    try:
        ...
        elif "DT Đang Thu Hoạch Cao Su (ha)" in data: land.harvest_area = parse_float_vn(data["DT Đang Thu Hoạch Cao Su (ha)"])
...
async def tien_nga_check_agricultural_land_handler(client, message: Message) -> None:
    ...
    land_code = args[1].upper()
    from app.models.business import AgriculturalLand, CropTreeLog
...
async def tien_nga_compare_harvest_handler(client, message: Message) -> None:
    ...
    try:
        ...
        for hh in households:
            ...
            harvest_area = land.harvest_area if land else 0
            ...
            rows.append({
                ...
                "avg_per_ha": total_actual / harvest_area if harvest_area > 0 else 0,
            })
        ...
        g_harvest = sum(r["harvest_area"] for r in rows)
        ...
        g_avg = g_act / g_harvest if g_harvest > 0 else 0

        g_vals = [g_harvest, g_act, g_dry, g_amt, g_paid, g_sav, g_avg]
        ...
        for r in rows:
            ...
            info_cell = ws_hh.cell(row=2, column=1, value=f"Mã Đất: {hh.land_code or '—'} | DT Thu Hoạch: {r['harvest_area']} ha | TB KL/ha: {round(r['avg_per_ha'], 2)} kg/ha")
            info_cell.alignment = Alignment(horizontal="center")
        ...
        await message.reply_document(
            ...
            caption=(
                ...
                f"Từ <b>{start_date.strftime('%d/%m/%Y')}</b> đến <b>{end_date.strftime('%d/%m/%Y')}</b>\n"
                f"Tổng hộ dân: <b>{len(rows)}</b> | Tổng DT Thu Hoạch: <b>{round(g_harvest, 2)} ha</b>"
            ),
            parse_mode=ParseMode.HTML
        )
        os.remove(tmp_path)
...
<i>Ghi chú: Đơn giá được tự động lấy từ thông tin hộ dân.</i>"""
    ...
    harvest_weight_str = data.get("Khối Lượng Mủ (Kg)", data.get("Khối Lượng Mủ", "0")).strip()
    ...
    harvest_weight = float(parse_float_vn(harvest_weight_str))
    if harvest_weight < 0:
    ...
    try:
        ...
        project = db.query(Projects).filter(Projects.project_name.in_(["Tiến Nga", "Thu Hoạch"])).first()
        if not project:
            await message.reply_text("⚠️ Không tìm thấy dự án Tiến Nga.", parse_mode=ParseMode.HTML)
            return
        ...
        cb_data = f"hc:{hh_code}:{use_land_code}:{day.strftime('%d%m%Y')}:{tree_count}:{int(unit_price)}:{member_chat_id}:{harvest_weight}"
        ...
        await client.send_message(
            ...
            text=(
                ...
                f"<b>Khối Lượng Mủ:</b> <code>{harvest_weight:,.2f} Kg</code>\n"
                f"<b>Đơn Giá:</b> <code>{fmt_money(unit_price)}</code>\n"
                f"<b>Thành Tiền:</b> <code>{fmt_money(total_amount)}</code>\n\n"
                f"<i>Vui lòng xác nhận hoặc từ chối yêu cầu này.</i>"
            ),
...
async def harvest_confirm_callback(client, callback_query):
    ...
    # hc:hh_code:land_code:day:tree_count:unit_price:member_chat_id:harvest_weight
    if len(parts) < 7:
        await callback_query.answer("⚠️ Dữ liệu không hợp lệ.", show_alert=True)
        return
    ...
    harvest_weight = float(parts[7]) if len(parts) > 7 else 0.0
    ...
    try:
        ...
        for hh_code, hv_list in harvest_map.items():
            ...
            for i, hv in enumerate(hv_list, 1):
                vals = [
                    ...
                    hv.land_code or "—",
                    hv.tree_count or 0,
                    round(hv.harvest_weight or 0, 2),
            ...
            sum_rubber = sum(hv.harvest_weight or 0 for hv in hv_list)
        ...
        if total_hh_count > 1:
            ...
            for idx, (hh_code, hv_list) in enumerate(harvest_map.items(), 1):
                ...
                s_rubber = sum(hv.harvest_weight or 0 for hv in hv_list)
...
async def rt_check_callback(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        logs = db.query(CropTreeLog).filter(
            CropTreeLog.land_code == land_code,
            CropTreeLog.crop_type == "cao_su"
        ).order_by(CropTreeLog.day.desc()).all()
...
# =========================================================================================
# QUẢN LÝ CÂY SẦU RIÊNG (THU HOẠCH)
# =========================================================================================

# --- Lệnh: /tien_nga_cay_sau_rieng ---
@bot.on_message(filters.command(["tien_nga_cay_sau_rieng"]) | filters.regex(r"^@\w+\s+/tien_nga_cay_sau_rieng\b"))
@require_user_type(UserType.OWNER, UserType.ADMIN)
@require_project_name("Tiến Nga", "Thu Hoạch")
@require_group_role("main")
@require_custom_title(CustomTitle.SUPER_MAIN, CustomTitle.MAIN_HARVEST)
...
async def tien_nga_durian_tree_handler(client, message: Message) -> None:
    lines = (message.text or "").strip().split("\n")
    if len(lines) >= 2:
        await tien_nga_durian_tree_form_handler(client, message)
        return

    args = await check_command_target(client, message.text, ["tien_nga_cay_sau_rieng"])
    if args is None: return

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("Trồng mới", callback_data="sr_action_PLANT")],
        [InlineKeyboardButton("Chặt cây", callback_data="sr_action_CUT")],
        [InlineKeyboardButton("Hủy", callback_data="sr_cancel")]
    ])
    await message.reply_text(
        "<b>QUẢN LÝ CÂY SẦU RIÊNG</b>\n\n"
        "Vui lòng chọn hành động:",
        reply_markup=keyboard,
        parse_mode=ParseMode.HTML
    )

@bot.on_callback_query(filters.regex(r"^sr_action_(PLANT|CUT)$"))
async def sr_action_callback(client, callback_query: CallbackQuery):
    action_type = callback_query.matches[0].group(1)
    action_label = "TRỒNG MỚI" if action_type == "PLANT" else "CHẶT CÂY"
    ...
    try:
        ...
        for land in lands:
            label = f"{land.land_code} - {land.land_name or land.address or ''}"
            buttons.append([InlineKeyboardButton(label, callback_data=f"sr_land_{action_type}_{land.land_code}")])
        buttons.append([InlineKeyboardButton("Hủy", callback_data="sr_cancel")])

        reply_markup = InlineKeyboardMarkup(buttons)
        await callback_query.message.edit_text(
            f"<b>{action_label} CÂY SẦU RIÊNG</b>\n\n"
            "Chọn đất trồng trọt:",
            reply_markup=reply_markup,
            parse_mode=ParseMode.HTML
        )
    except Exception as e:
        LogError(f"Error in sr_action_callback: {e}", LogType.SYSTEM_STATUS)
        await callback_query.answer("❌ Có lỗi xảy ra.", show_alert=True)
    finally:
        db.close()

@bot.on_callback_query(filters.regex(r"^sr_land_(PLANT|CUT)_(.+)$"))
async def sr_land_callback(client, callback_query: CallbackQuery):
    action_type = callback_query.matches[0].group(1)
    land_code = callback_query.matches[0].group(2)
    action_label = "TRỒNG MỚI" if action_type == "PLANT" else "CHẶT CÂY"
    ...
    try:
        ...
        if not land:
            await callback_query.answer("⚠️ Không tìm thấy đất.", show_alert=True)
            return
        ...
        form = (
            f"<b>FORM {action_label} CÂY SẦU RIÊNG</b>\n\n"
            f"Đất: <b>{land.land_name or land.land_code}</b> (<code>{land.land_code}</code>)\n"
            f"Cây đang thu hoạch: <b>{land.harvesting_trees or 0}</b>\n"
            f"Cây đang trồng: <b>{land.planting_trees or 0}</b>\n\n"
            f"Sao chép form dưới đây, điền thông tin và gửi lại:\n\n"
            f"<pre>/tien_nga_cay_sau_rieng\n"
            f"Loại: {action_label}\n"
            f"Mã Đất: {land.land_code}\n"
            f"Ngày: {today_str}\n"
            f"Số Lượng: 0\n"
        ...
        LogError(f"Error in sr_land_callback: {e}", LogType.SYSTEM_STATUS)
        await callback_query.answer("❌ Có lỗi xảy ra.", show_alert=True)
    finally:
        db.close()

@bot.on_callback_query(filters.regex(r"^sr_cancel$"))
async def sr_cancel_callback(client, callback_query: CallbackQuery):
    await callback_query.message.delete()
...
async def tien_nga_durian_tree_form_handler(client, message: Message) -> None:
    ...
    try:
        ...
        await message.reply_text(
            f"{action_icon} <b>{action_label} CÂY SẦU RIÊNG THÀNH CÔNG!</b>\n\n"
            f"<b>Đất:</b> {land.land_name or land.land_code} (<code>{land_code}</code>)\n"
            f"<b>Ngày:</b> {day.strftime('%d/%m/%Y')}\n"
            f"<b>Số Lượng:</b> {quantity} cây\n"
            f"<b>Người TH:</b> {executor or '—'}\n"
        ...
        LogInfo(f"[TienNga] DurianTree {action_type}: {quantity} trees on {land_code} by {executor}", LogType.SYSTEM_STATUS)

    except Exception as e:
        db.rollback()
        LogError(f"Error in durian tree form handler: {e}\n{traceback.format_exc()}", LogType.SYSTEM_STATUS)
        await message.reply_text(f"❌ Có lỗi xảy ra: {e}", parse_mode=ParseMode.HTML)
    finally:
        db.close()


# --- Lệnh: /tien_nga_kt_cay_sau_rieng ---
@bot.on_message(filters.command(["tien_nga_kt_cay_sau_rieng"]) | filters.regex(r"^@\w+\s+/tien_nga_kt_cay_sau_rieng\b"))
@require_user_type(UserType.OWNER, UserType.ADMIN)
@require_project_name("Tiến Nga", "Thu Hoạch")
@require_group_role("main")
@require_custom_title(CustomTitle.SUPER_MAIN, CustomTitle.MAIN_HARVEST)
...
async def tien_nga_check_durian_tree_handler(client, message: Message) -> None:
    ...
    try:
        ...
        for land in lands:
            label = f"{land.land_code} - {land.land_name or land.address or ''}"
            buttons.append([InlineKeyboardButton(label, callback_data=f"sr_check_{land.land_code}")])
        buttons.append([InlineKeyboardButton("Hủy", callback_data="sr_cancel")])

        reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply_text(
            "<b>KIỂM TRA CÂY SẦU RIÊNG</b>\n\n"
            "Chọn đất cần kiểm tra:",
            reply_markup=reply_markup,
            parse_mode=ParseMode.HTML
        )
    except Exception as e:
        LogError(f"Error in check durian tree handler: {e}", LogType.SYSTEM_STATUS)
        await message.reply_text("❌ Có lỗi hệ thống.", parse_mode=ParseMode.HTML)
    finally:
        db.close()

@bot.on_callback_query(filters.regex(r"^sr_check_(.+)$"))
async def sr_check_callback(client, callback_query: CallbackQuery):
    land_code = callback_query.matches[0].group(1)
    ...
    try:
        ...
        logs = db.query(CropTreeLog).filter(
            CropTreeLog.land_code == land_code,
            CropTreeLog.crop_type == "sau_rieng"
        ).order_by(CropTreeLog.day.desc()).all()
        ...
        if not plant_logs:
            ...
            for idx, log in enumerate(plant_logs[:20], 1):
                day_str = log.day.strftime("%d/%m/%Y") if log.day else "—"
                text += f"  {idx}. {day_str} | {log.quantity} cây | {log.executor or '—'}"
                if log.notes:
                    text += f" | {log.notes}"
                text += "\n"
            if len(plant_logs) > 20:
        ...
        LogError(f"Error in sr_check_callback: {e}\n{traceback.format_exc()}", LogType.SYSTEM_STATUS)
        await callback_query.answer("❌ Có lỗi xảy ra.", show_alert=True)
    finally:
        db.close()


# --- Lệnh: /tien_nga_thu_hoach_sau_rieng (main) ---
@bot.on_message(filters.command(["tien_nga_thu_hoach_sau_rieng"]) | filters.regex(r"^@\w+\s+/tien_nga_thu_hoach_sau_rieng\b"))
@require_user_type(UserType.OWNER, UserType.ADMIN)
@require_project_name("Tiến Nga", "Thu Hoạch")
@require_group_role("main")
@require_custom_title(CustomTitle.SUPER_MAIN, CustomTitle.MAIN_HARVEST)
async def tien_nga_durian_harvest_main_handler(client, message: Message) -> None:
    ...
    if len(lines) < 2:
        if len(lines[0].split()) < 2:
            await message.reply_text(
                "<b>THU HOẠCH SẦU RIÊNG</b>\n\n"
                "Cú pháp: <code>/tien_nga_thu_hoach_sau_rieng [Mã Hộ Dân]</code>\n\n"
                "VD: <code>/tien_nga_thu_hoach_sau_rieng HD001</code>",
                parse_mode=ParseMode.HTML
            )
            return

        hh_code = lines[0].split()[1].upper()
        db = SessionLocal()
        try:
            hh = db.query(Households).filter(Households.household_code == hh_code, Households.status == "ACTIVE").first()
            if not hh:
            ...
            today = dt_cls.now().strftime("%d/%m/%Y")
            form = f"""<b>FORM THU HOẠCH SẦU RIÊNG</b>
...
<pre>/tien_nga_thu_hoach_sau_rieng
Mã Hộ Dân: {hh_code}
Mã Đất: {hh.land_code or ''}
Ngày: {today}
Khối Lượng (Kg): 0
Đơn Giá (VNĐ/Kg): 0</pre>

<i>Ghi chú: Nhập đơn giá trực tiếp cho mỗi lần thu hoạch.</i>"""
            await message.reply_text(form, parse_mode=ParseMode.HTML)
        finally:
            db.close()
        return
    ...
    land_code = data.get("Mã Đất", "").strip().upper()
    day_str = data.get("Ngày", "").strip()
    weight_str = data.get("Khối Lượng (Kg)", data.get("Khối Lượng", "0")).strip()
    price_str = data.get("Đơn Giá (VNĐ/Kg)", data.get("Đơn Giá", "0")).strip()
    ...
    harvest_weight = float(parse_float_vn(weight_str))
    if harvest_weight <= 0:
        await message.reply_text("⚠️ <b>Khối Lượng</b> phải lớn hơn 0.", parse_mode=ParseMode.HTML)
        return

    unit_price = float(parse_float_vn(price_str))
    if unit_price < 0:
        await message.reply_text("⚠️ <b>Đơn Giá</b> không được âm.", parse_mode=ParseMode.HTML)
        return
    ...
    try:
        ...
        new_harvest = DailyHarvest(
            ...
            day=day,
            household_code=hh_code,
            land_code=use_land_code or None,
            tree_count=0,
            harvest_weight=harvest_weight,
            unit_price=unit_price,
            total_amount=total_amount,
        ...
        await message.reply_text(
            f"✅ <b>THU HOẠCH SẦU RIÊNG THÀNH CÔNG!</b>\n\n"
            f"<b>Hộ Dân:</b> {hh.fullname or hh_code} (<code>{hh_code}</code>)\n"
            f"<b>Mã Đất:</b> {land_display}\n"
            f"<b>Ngày:</b> {day.strftime('%d/%m/%Y')}\n"
            f"<b>Khối Lượng:</b> <code>{harvest_weight:,.2f} Kg</code>\n"
            f"<b>Đơn Giá:</b> <code>{fmt_money(unit_price)}</code>\n"
            f"<b>Thành Tiền:</b> <code>{fmt_money(total_amount)}</code>\n"
            f"<b>Công Nợ Mới:</b> <code>{fmt_money(hh.total_debt)}</code>",
            parse_mode=ParseMode.HTML
        ...
        db.rollback()
        LogError(f"Error creating durian harvest: {e}", LogType.SYSTEM_STATUS)
        await message.reply_text("❌ Có lỗi hệ thống.", parse_mode=ParseMode.HTML)
    finally:
        db.close()


# --- Lệnh: /tien_nga_thu_hoach_sr_hang_ngay (member) ---
@bot.on_message(filters.command(["tien_nga_thu_hoach_sr_hang_ngay"]) | filters.regex(r"^@\w+\s+/tien_nga_thu_hoach_sr_hang_ngay\b"))
@require_user_type(UserType.OWNER, UserType.ADMIN, UserType.MEMBER)
@require_project_name("Tiến Nga", "Thu Hoạch")
@require_group_role("member")
@require_custom_title(CustomTitle.SUPER_MAIN, CustomTitle.MAIN_HARVEST, CustomTitle.MEMBER_HARVEST)
...
async def tien_nga_durian_harvest_member_handler(client, message: Message) -> None:
    ...
    if len(lines) < 2:
        if len(args) < 2:
            await message.reply_text(
                "<b>THU HOẠCH SẦU RIÊNG HÀNG NGÀY</b>\n\n"
                "Cú pháp: <code>/tien_nga_thu_hoach_sr_hang_ngay [Mã Hộ Dân]</code>\n\n"
                "VD: <code>/tien_nga_thu_hoach_sr_hang_ngay HD001</code>",
                parse_mode=ParseMode.HTML
            )
            return
...
<pre>/tien_nga_thu_hoach_sr_hang_ngay
Mã Hộ Dân: {hh_code}
Mã Đất: {hh.land_code or ''}
Ngày: {today}
Khối Lượng (Kg): 0
...
<i>Ghi chú: Nhập đơn giá trực tiếp cho mỗi lần thu hoạch.</i>"""
    ...
    try:
        ...
        # callback data: sc:hh_code:land_code:day:weight:price:member_chat_id
        cb_data = f"sc:{hh_code}:{use_land_code}:{day.strftime('%d%m%Y')}:{harvest_weight}:{int(unit_price)}:{member_chat_id}"
        cb_deny = f"sd:{hh_code}:{member_chat_id}"
        ...
        await client.send_message(
            chat_id=int(main_chat.chat_id),
            text=(
                f"🔔 <b>YÊU CẦU XÁC NHẬN THU HOẠCH SẦU RIÊNG</b>\n\n"
                f"<b>Người gửi:</b> {user_display}\n"
                f"<b>Hộ Dân:</b> {hh.fullname or hh_code} (<code>{hh_code}</code>)\n"
                f"<b>Mã Đất:</b> {land_display}\n"
                f"<b>Ngày:</b> {day.strftime('%d/%m/%Y')}\n"
        ...
        await message.reply_text(
            f"<b>Đã gửi yêu cầu thu hoạch sầu riêng lên nhóm chính!</b>\n\n"
            f"<b>Hộ Dân:</b> {hh.fullname or hh_code}\n"
            f"<b>Khối Lượng:</b> <code>{harvest_weight:,.2f} Kg</code>\n"
            f"<b>Đơn Giá:</b> <code>{fmt_money(unit_price)}</code>\n"
            f"<b>Thành Tiền:</b> <code>{fmt_money(total_amount)}</code>\n\n"
        ...
        db.rollback()
        LogError(f"Error creating durian harvest request: {e}", LogType.SYSTEM_STATUS)
        await message.reply_text("❌ Có lỗi hệ thống.", parse_mode=ParseMode.HTML)
    finally:
        db.close()

# Callback: Xác nhận thu hoạch sầu riêng
@bot.on_callback_query(filters.regex(r"^sc:"))
async def durian_harvest_confirm_callback(client, callback_query):
    from datetime import datetime as dt_cls
    from app.models.business import Households, AgriculturalLand, DailyHarvest
    import uuid as uuid_lib

    parts = callback_query.data.split(":")
    # sc:hh_code:land_code:day:weight:price:member_chat_id
    if len(parts) < 7:
        await callback_query.answer("⚠️ Dữ liệu không hợp lệ.", show_alert=True)
        return
    ...
    try:
        ...
        await callback_query.message.edit_text(
            f"✅ <b>ĐÃ XÁC NHẬN THU HOẠCH SẦU RIÊNG</b>\n\n"
            f"<b>Hộ Dân:</b> {hh.fullname or hh_code} (<code>{hh_code}</code>)\n"
            f"<b>Mã Đất:</b> {land_display}\n"
            f"<b>Ngày:</b> {day.strftime('%d/%m/%Y')}\n"
            f"<b>Khối Lượng:</b> <code>{harvest_weight:,.2f} Kg</code>\n"
        ...
        try:
            await client.send_message(
                chat_id=int(member_chat_id),
                text=(
                    f"✅ <b>Thu Hoạch Sầu Riêng Đã Được Xác Nhận!</b>\n\n"
                    f"<b>Hộ Dân:</b> {hh.fullname or hh_code} (<code>{hh_code}</code>)\n"
                    f"<b>Ngày:</b> {day.strftime('%d/%m/%Y')}\n"
                    f"<b>Khối Lượng:</b> <code>{harvest_weight:,.2f} Kg</code>\n"
                    f"<b>Đơn Giá:</b> <code>{fmt_money(unit_price)}</code>\n"
...
# Callback: Từ chối thu hoạch sầu riêng
@bot.on_callback_query(filters.regex(r"^sd:"))
async def durian_harvest_deny_callback(client, callback_query):
    parts = callback_query.data.split(":")
    if len(parts) < 3:
        await callback_query.answer("⚠️ Dữ liệu không hợp lệ.", show_alert=True)
        return
    ...
    await callback_query.message.edit_text(
        f"❌ <b>ĐÃ TỪ CHỐI THU HOẠCH SẦU RIÊNG</b>\n\n"
        f"<b>Hộ Dân:</b> <code>{hh_code}</code>\n"
        f"<b>Từ chối bởi:</b> {denier}",
        parse_mode=ParseMode.HTML
    )
    ...
    try:
        await client.send_message(
            chat_id=int(member_chat_id),
            text=(
                f"❌ <b>Yêu Cầu Thu Hoạch Sầu Riêng Bị Từ Chối</b>\n\n"
                f"<b>Hộ Dân:</b> <code>{hh_code}</code>\n"
                f"<b>Từ chối bởi:</b> {denier}\n\n"
                f"<i>Vui lòng kiểm tra lại và gửi yêu cầu mới nếu cần.</i>"
            ),
...
# --- Lệnh: /tien_nga_kt_thu_hoach_sr ---
@bot.on_message(filters.command(["tien_nga_kt_thu_hoach_sr"]) | filters.regex(r"^@\w+\s+/tien_nga_kt_thu_hoach_sr\b"))
@require_user_type(UserType.OWNER, UserType.ADMIN)
@require_project_name("Tiến Nga", "Thu Hoạch")
@require_group_role("main")
@require_custom_title(CustomTitle.SUPER_MAIN, CustomTitle.MAIN_HARVEST)
async def tien_nga_check_durian_harvest_handler(client, message: Message) -> None:
    ...
    if len(parts) >= 2:
        ...
        if not date_match:
            await message.reply_text("⚠️ Định dạng không hợp lệ.\nVD: <code>/tien_nga_kt_thu_hoach_sr 01/04/2026 - 27/04/2026</code>", parse_mode=ParseMode.HTML)
            return

        try:
            start_date = dt_cls.strptime(date_match.group(1), "%d/%m/%Y").date()
            end_date = dt_cls.strptime(date_match.group(2), "%d/%m/%Y").date()
        except Exception:
            await message.reply_text("⚠️ Định dạng ngày không hợp lệ.", parse_mode=ParseMode.HTML)
            return
        ...
        land_code = args_text[:date_match.start()].strip().upper() or None
        await _do_check_durian_harvest(client, message, start_date, end_date, land_code=land_code)
        return
    ...
    first_day_str = datetime.datetime.now().replace(day=1).strftime('%d/%m/%Y')
    await message.reply_text(
        "<b>KIỂM TRA THU HOẠCH SẦU RIÊNG</b>\n\n"
        f"Cú pháp: <code>/tien_nga_kt_thu_hoach_sr {first_day_str} - {today_str}</code>\n"
        f"Hoặc: <code>/tien_nga_kt_thu_hoach_sr [Mã Đất] {first_day_str} - {today_str}</code>",
        parse_mode=ParseMode.HTML
    )


async def _do_check_durian_harvest(client, message, start_date, end_date, land_code=None):
    from app.models.business import Households, DailyHarvest
    from app.db.session import SessionLocal
    db = SessionLocal()
    try:
        ...
        harvests = db.query(DailyHarvest).filter(
            DailyHarvest.household_code.in_(hh_codes),
            DailyHarvest.crop_type == "sau_rieng",
            DailyHarvest.day >= start_date,
            DailyHarvest.day <= end_date
        ).order_by(DailyHarvest.day).all()
        ...
        if not harvests:
            await message.reply_text(
                f"📋 Không có dữ liệu thu hoạch sầu riêng từ <b>{start_date.strftime('%d/%m/%Y')}</b> đến <b>{end_date.strftime('%d/%m/%Y')}</b>.",
                parse_mode=ParseMode.HTML
            )
            return
        ...
        headers = ["STT", "Ngày", "Mã Đất", "KL Trái (Kg)", "Đơn Giá", "Thành Tiền"]
        ...
        for hh_code, hv_list in harvest_map.items():
            ...
            title_cell = ws.cell(row=1, column=1,
                value=f"THU HOẠCH SẦU RIÊNG: {hh_code} - {hh.fullname or ''}")
            title_cell.font = Font(bold=True, size=14)
            title_cell.alignment = Alignment(horizontal="center")

            ws.merge_cells("A2:F2")
            info_cell = ws.cell(row=2, column=1,
                value=f"Từ {start_date.strftime('%d/%m/%Y')} đến {end_date.strftime('%d/%m/%Y')} | Mã Đất: {hh.land_code or '—'}")
            info_cell.alignment = Alignment(horizontal="center")
            ...
            sum_weight = sum(hv.harvest_weight or 0 for hv in hv_list)
            ...
            ws.cell(row=c_row, column=4, value=round(sum_weight, 2)).number_format = '#,##0.00'
        ...
        if total_hh_count > 1:
            ...
            title_cell = ws_sum.cell(row=1, column=1,
                value=f"TỔNG HỢP THU HOẠCH SẦU RIÊNG - Từ {start_date.strftime('%d/%m/%Y')} đến {end_date.strftime('%d/%m/%Y')}")
            title_cell.font = Font(bold=True, size=14)
            title_cell.alignment = Alignment(horizontal="center")

            sum_headers = ["STT", "Mã HD", "Họ Tên", "Tổng KL (Kg)", "Tổng Thành Tiền"]
            for col_idx, hd in enumerate(sum_headers, 1):
                cell = ws_sum.cell(row=3, column=col_idx, value=hd)
                cell.font = hdr_font
                cell.fill = hdr_fill
            ...
            for idx, (hh_code, hv_list) in enumerate(harvest_map.items(), 1):
                ...
                if not hh:
                    continue
                s_weight = sum(hv.harvest_weight or 0 for hv in hv_list)
                s_amount = sum(hv.total_amount or 0 for hv in hv_list)
                g_weight += s_weight
                g_amount += s_amount

                vals = [idx, hh_code, hh.fullname or "", round(s_weight, 2), round(s_amount, 0)]
                for col_idx, v in enumerate(vals, 1):
                    cell = ws_sum.cell(row=s_row, column=col_idx, value=v)
                    cell.border = thin_border
                    if col_idx == 4:
            ...
            for col_idx in range(1, 6):
                ...
                cell.border = thin_border
                cell.font = total_font
            ws_sum.cell(row=s_row, column=4, value=round(g_weight, 2)).number_format = '#,##0.00'
            ws_sum.cell(row=s_row, column=5, value=round(g_amount, 0)).number_format = '#,##0'

            sum_widths = [6, 12, 22, 16, 18]
            for i, w in enumerate(sum_widths, 1):
                ws_sum.column_dimensions[openpyxl.utils.get_column_letter(i)].width = w
        ...
        caption = (
            f"<b>KIỂM TRA THU HOẠCH SẦU RIÊNG</b>\n"
            f"Từ <b>{start_date.strftime('%d/%m/%Y')}</b> đến <b>{end_date.strftime('%d/%m/%Y')}</b>\n"
            f"Số hộ dân: <b>{total_hh_count}</b>\n"
            f"Tổng thành tiền: <b>{fmt_money(grand_total)}</b>"
        )
        ...
        await message.reply_document(
            document=tmp_path,
            file_name=f"kt_thu_hoach_sr_{start_date.strftime('%d%m%Y')}_{end_date.strftime('%d%m%Y')}.xlsx",
            caption=caption,
            parse_mode=ParseMode.HTML
        )
        os.remove(tmp_path)
    except Exception as e:
        LogError(f"Error in check durian harvest: {e}", LogType.SYSTEM_STATUS)
        await message.reply_text("❌ Có lỗi hệ thống.", parse_mode=ParseMode.HTML)
    finally:
        db.close()
